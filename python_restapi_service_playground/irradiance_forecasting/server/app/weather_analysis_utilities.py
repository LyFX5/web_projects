...
class DataBaseWeatherSource(WeatherDataSource):
    ...
    def provide_forecasted_data(self, n_days: int) -> pd.DataFrame:
        ...
        print(f"[provide_forecasted_data] {(h, m)=}")
        if ((h % API_REQUEST_FREQUENCY) == 0) and (m < 10):
            ...
            try:
                ...
                print(f"[provide_forecasted_data] data has been loaded from database.")
            except Exception as exception:
                print(exception)
                return
    ...
    def provide_forecasted_data_by_coords(self, n_days, latitude, longitude) -> pd.DataFrame:
        return self.api_wrapper.provide_forecasted_data_by_coords(n_days, latitude, longitude)
